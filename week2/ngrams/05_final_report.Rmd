...
yearcounts <- read_tsv(here('year_counts.tsv'),
                    col_names = c('term', 'year', 'volume', 'book_count'))

totalcounts <- read_csv(here('total_counts.csv'),
...
There are `r count(yearcounts)$n` lines in the yearcounts dataframe, and `r count(totalcounts)$n` lines in the totalcounts dataframe.
...
counts <- left_join(yearcounts, totalcounts, by = "year") %>%
...
counts2 <- counts %>% filter(term == "1883" | term == "1910" | term == "1950") %>%
  mutate(term = as.character(term))

counts2 %>% 
  ggplot() + geom_line(aes(x = year, y = prop_mentions, color = as.factor(term))) +
  scale_y_continuous(label = percent) + xlim(1850, 2000) + xlab("Year") +
  ylab("Frequency")
...
My graph is similar to the ngram viewer graph.
...
counts2 %>% 
  ggplot() + geom_line(aes(x = year, y = volume, color = term)) +
  scale_y_continuous(label = comma) + xlim(1850, 2012) + xlab("Year") +
  ylab("Frequency")
...
I reject Michel et al.'s conclusion.  When looking at percentages, it looks like people are forgetting things faster over time, but when looking at just counts, it is clear that that is not the case, all of the years are mentioned more as time goes on (probably because more books are being published.)
...
peaks <- counts %>%
  group_by(term) %>% filter(year >= 1850) %>%
  filter(prop_mentions == max(prop_mentions)) %>%
  mutate(half_prop = .5 * prop_mentions) %>%
  mutate(peak_year = year) %>% ungroup()
...
counts <- counts %>% select(term, year, prop_mentions)
peaks <- peaks %>% select(term, peak_year, half_prop)

rejoint <- left_join(counts, peaks) %>%
  filter(term >= 1875 & term <= 1975) %>%
  filter(year >= peak_year) %>%
  group_by(term) %>%
  filter(prop_mentions < half_prop) %>%
...
rejoint %>% ggplot() + 
  geom_point(aes(x = term, y = half_life)) + 
  geom_smooth(aes(x = term, y = half_life))
...
My plot is not similar to the one in the inset of figure 3a that was created from the version 1 data.
...
rejoint %>% select(term, half_life) %>%
filter(half_life == min(half_life))

rejoint %>% select(term, half_life) %>%
filter(half_life == max(half_life))

1877, 1878, and 1942 had the shortest half-lifes, 4 years. 1900 had the longest half-life, 19 years.
